<template>
  <DefaultLayout>
    <main class="relative grid grow place-items-center overflow-hidden py-8">
      <div
        v-if="is404"
      ...
      >
        <div
          ref="ball"
          class="from-primary-300 to-primary-400 absolute bottom-1 size-4 rounded-full bg-radial drop-shadow-lg will-change-transform md:size-8"
        ></div>
      ...
      <div
        ref="infoContainer"
        class="absolute flex h-full flex-col items-center justify-center gap-4 px-8 text-center text-balance max-md:max-w-sm"
        :class="{ invisible: is404 }"
      >
        <h1 class="h2">{{ is404 ? "Oh no, where did the page go?!" : "An unexpected error occurred" }}</h1>
        <p>
          {{
            is404
        ...
        <StyledButton secondary class="mt-4" @click="navigateTo(useRequestHeader('Referrer') || '/')">
          Go back
        </StyledButton>
      </div>
    </main>
...
onMounted(() => {
  ...
  const cups = Array.from(document.getElementsByClassName("cup")) as SVGElement[];
  ...
  function animateCup(cup: SVGElement, xPosition: number, onComplete?: () => void) {
